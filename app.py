...
def save_image(image_file) -> str:
    ...
    image_filename = f"{timestamp}_{secure_filename(image_file.filename)}"
...
def conversation(user_id: int):
    ...
    if request.method == "POST":
        ...
        if not receiver or receiver.id == current_user.id:
            flash("Please choose a valid user.", "danger")
            return redirect(url_for("conversation", user_id=user_id))

        if not body:
            flash("Message cannot be empty.", "danger")
            return redirect(url_for("conversation", user_id=receiver.id))

        message = Message(sender_id=current_user.id, receiver_id=receiver.id, item_id=item_id, body=body)
